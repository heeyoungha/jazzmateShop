...
특히 DB client 같은 app-scoped 필수 리소스가 누락된 상태로 `RecommendationService`나 Repository가 조립되면 안 된다.

검증 기준:

- `AlbumEmbeddingRepository(database=None)`은 정상 생성 경로가 아니며, 설정 누락을 설명하는 명확한 예외를 발생시킨다.
- `get_recommendation_service(request)`는 `request.app.state.database`의 DB client를 Repository에 주입한다.
- `request.app.state.database`가 없거나 `None`이면 dependency provider가 명확한 설정 오류를 발생시킨다.
...
| `tests/unit/test_dependencies.py` | FastAPI dependency provider가 app state DB client를 Service/Repository에 주입 |
...
| app state에 DB client 있음 | provider가 해당 client로 Repository를 조립 |
| app state에 DB client 없음 | provider가 설정 누락 예외 발생 |
| Service 테스트에서 fake repository 주입 | 실제 DB client 없이 유스케이스 분기 검증 가능 |

이 테스트는 실제 Supabase 네트워크 연결을 하지 않는다.
fake DB client를 사용하되, fake가 production wiring 위치에 주입되는지 검증한다.
