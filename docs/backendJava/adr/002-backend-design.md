...
4. AI 처리
   - 완료 → FastAPI가 `POST /{reviewId}/recommendations` 콜백 → `saveAll()` 일괄 저장 → status=COMPLETED
   - 실패 → status=FAILED (RecommendationEventListener에서 처리)

---

## Decision 3: 추천 결과는 saveAll()로 하나의 트랜잭션에서 일괄 저장

### Context

FastAPI는 추천 앨범 N건을 하나의 콜백 요청으로 Spring Boot에 전달한다.
각 추천 결과를 Service에서 개별 `save()` 호출로 저장하면 요청 단위의 저장 의도가 분산되고, 중복 실패 처리도 흐려진다.

### Decision

추천 결과는 `RecommendAlbum` 리스트로 변환한 뒤 `saveAll()`로 저장한다.
...
**Decision 3**
- 추천 저장은 `saveAll()`로 하나의 트랜잭션에서 일괄 처리
