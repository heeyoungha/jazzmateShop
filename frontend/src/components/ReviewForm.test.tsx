...
describe("ReviewForm", () => {
  it("필수 필드가 렌더링된다", () => {
    ...
    expect(screen.getByLabelText("앨범명")).toBeInTheDocument();
  ...
  it("선택 필드가 렌더링된다", () => {
    ...
    expect(screen.getByLabelText("감상문 공개")).toBeInTheDocument();
  });
  ...
  it("필수 필드 없이 제출하면 유효성 에러가 표시된다", async () => {
    ...
    expect(screen.getByText("앨범명은 필수입니다.")).toBeInTheDocument();
  ...
  it("공백만 입력한 필수 필드는 유효성 에러가 표시된다", async () => {
    ...
    await user.type(screen.getByLabelText("앨범명"), "   ");
  ...
  it("선택 필드 없이도 제출이 가능하다", async () => {
    ...
    await user.type(screen.getByLabelText("앨범명"), "So What");
  ...
  it("선택 필드를 포함하면 해당 값이 onSubmit에 전달된다", async () => {
    ...
    await user.click(screen.getByLabelText("감상문 공개"));
    await user.click(screen.getByRole("button", { name: "감상문 저장하기" }));
